...
test("interrupts an aborted turn and does not persist an incomplete mapping", async () => {
  await withService(async ({ service, client, database }) => {
    ...
    assert.deepEqual(
      client.requests.find((request) => request.method === "thread/delete")
        ?.params,
      { threadId: "thread-abort" }
    );
...
test("account change during thread creation prevents the prompt from being sent", async () => {
  await withService(async ({ service, client }) => {
    ...
    assert.deepEqual(
      client.requests.find((request) => request.method === "thread/delete")
        ?.params,
      { threadId: "thread-account-transition" }
    );
  });
});

test("retains a cleanup mapping when an interrupted native thread cannot be deleted", async () => {
  await withService(async ({ service, client, database }) => {
    client.handlers.set("account/read", () => connectedAccount());
    client.handlers.set("thread/start", () => ({
      thread: { id: "thread-cleanup-retry" },
    }));
    client.handlers.set("turn/start", () => ({
      turn: { id: "turn-cleanup-retry" },
    }));
    client.handlers.set("thread/delete", () => {
      throw new Error("runtime unavailable");
    });
    ...
    const pending = service.runTurn({
      chatId: "chat-cleanup-retry",
      model: "gpt-5-codex",
      instructions: "",
      messages: [chatMessage("user-cleanup-retry", "user", "Stop")],
      onEvent: () => undefined,
    });
    await waitForRequest(client, "turn/start");
    client.emitNotification("turn/completed", {
      threadId: "thread-cleanup-retry",
      turn: { id: "turn-cleanup-retry", status: "interrupted" },
    });

    assert.equal((await pending).status, "interrupted");
    assert.deepEqual(database.getCodexChatThread("chat-cleanup-retry"), {
      chatId: "chat-cleanup-retry",
      threadId: "thread-cleanup-retry",
      lastUserMessageId: null,
      createdAt:
        database.getCodexChatThread("chat-cleanup-retry")?.createdAt,
      updatedAt:
        database.getCodexChatThread("chat-cleanup-retry")?.updatedAt,
    });
  });
});
